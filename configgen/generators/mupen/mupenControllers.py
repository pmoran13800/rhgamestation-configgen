...
mupenSettings = UnixSettings(rhgamestationFiles.mupenCustom, separator=' ')
...
def getMupenMappingFile():
    if os.path.exists(rhgamestationFiles.mupenMappingUser):
        return rhgamestationFiles.mupenMappingUser
    else:
        return rhgamestationFiles.mupenMappingSystem
...
def writeControllersConfig(controllers):
	if os.path.isfile(rhgamestationFiles.mupenInput):
		os.remove(rhgamestationFiles.mupenInput)
...
def writeToIni(controller, config):
	Config.read(rhgamestationFiles.mupenInput)
	...
	# Open file
	cfgfile = open(rhgamestationFiles.mupenInput,'w+')
