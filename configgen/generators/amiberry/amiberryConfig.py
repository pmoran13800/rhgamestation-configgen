...
def initMountpoint(mountPoint) :
    ...
    # Unpatched version
    # oldAmiberryPath="/rhgamestation/share/emulateurs/amiga/amiberry"
...
def hasCD32Kickstarts() : return os.path.exists(os.path.join(rhgamestationFiles.BIOS,"CD32ext.rom")) and os.path.exists(os.path.join(rhgamestationFiles.BIOS,"kick31CD32.rom"))

def hasKickstarts(hardware,whdl) :
    if hardware == 'amiga1200' and os.path.exists(os.path.join(rhgamestationFiles.BIOS,"kick31.rom")) :
        return True
    elif hardware == 'amiga600' :
        if whdl :
            return os.path.exists(os.path.join(rhgamestationFiles.BIOS,"kick20.rom"))
        else :
            return os.path.exists(os.path.join(rhgamestationFiles.BIOS,"kick13.rom"))
...
def generateKickstartPath(fUaeConfig, amigaHardware) :
    if  amigaHardware == "amiga1200" :
        fUaeConfig.save("kickstart_rom_file",os.path.join(rhgamestationFiles.BIOS,"kick31.rom"))
    else :
        fUaeConfig.save("kickstart_rom_file",os.path.join(rhgamestationFiles.BIOS,"kick13.rom"))
        
def generateKickstartPathWHDL(fUaeConfig, amigaHardware) :
    fUaeConfig.save("rom_path",rhgamestationFiles.BIOS)
    if  amigaHardware == "amiga1200" :
        fUaeConfig.save("kickstart_rom_file",os.path.join(rhgamestationFiles.BIOS,"kick31.rom"))
    else :
        fUaeConfig.save("kickstart_rom_file",os.path.join(rhgamestationFiles.BIOS,"kick20.rom"))
        
def generateKickstartPathCD32(fUaeConfig, amigaHardware) :
    fUaeConfig.save("rom_path",rhgamestationFiles.BIOS)
    fUaeConfig.save("kickstart_rom_file",os.path.join(rhgamestationFiles.BIOS,"kick31CD32.rom"))
    fUaeConfig.save("kickstart_ext_rom_file",os.path.join(rhgamestationFiles.BIOS,"CD32ext.rom"))
    fUaeConfig.save("flash_file",os.path.join(rhgamestationFiles.BIOS,"cd32.nvr"))
...
def generateGraphicConf(fUaeConfig) :
    ...
    # pandora.floppy_path=/rhgamestation/share/emulateurs/amiga/amiberry/disks/
    # pandora.hardfile_path=/rhgamestation/share/roms/amiga/
    # ; host-specific
