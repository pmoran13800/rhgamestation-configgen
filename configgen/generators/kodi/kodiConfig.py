...
def getKodiMappingFile():
    if os.path.exists(rhgamestationFiles.kodiMappingUser):
        return rhgamestationFiles.kodiMappingUser
    else:
        return rhgamestationFiles.kodiMappingSystem
...
def writeKodiConfig(controllersFromES):
    ...
    if len(controllersFromES) == 0:
        return
    directory = os.path.dirname(rhgamestationFiles.kodiJoystick)
    if not os.path.exists(directory):
        os.makedirs(directory)
    kodiJoy = open(rhgamestationFiles.kodiJoystick, "w")
