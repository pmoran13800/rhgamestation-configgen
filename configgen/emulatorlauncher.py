...
generators = {
    ...
    'kodi': KodiGenerator(),
    'linapple': LinappleGenerator(os.path.join(rhgamestationFiles.HOME_INIT, '.linapple'),
                                  os.path.join(rhgamestationFiles.HOME, '.linapple')),
...
def main(args):
    ...
    if systemName in emulators:
        ...
        # Save dir
        dirname = os.path.join(rhgamestationFiles.savesDir, system.name)
        if not os.path.exists(dirname):
            os.makedirs(dirname)
        
        
        if system.config['emulator'] not in rhgamestationFiles.rhgamestationBins:
        ...
        # command.array.insert(0, rhgamestationFiles.rhgamestationBins[system.config['emulator']])
        print(command)
